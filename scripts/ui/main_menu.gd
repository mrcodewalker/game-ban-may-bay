...
func _ready() -> void:
	...
	if has_node("CampaignPanel/BackButton"):
		$CampaignPanel/BackButton.pressed.connect(show_main_panel)
	if has_node("ShopPanel/BackButton"):
		$ShopPanel/BackButton.pressed.connect(show_main_panel)
	if has_node("ControlsPanel/CloseButton"):
		$ControlsPanel/CloseButton.pressed.connect(show_main_panel)

	# Connect Campaign Play Buttons
...
func update_ui_data() -> void:
	if coins_label_main:
		coins_label_main.text = "💰 COINS: %d" % GameManager.coins
	if coins_label_shop:
		coins_label_shop.text = "💰 TOTAL COINS: %d" % GameManager.coins
	...
	for i in range(5):
		...
		if has_node(card_path):
			...
			if is_unlocked:
				...
				if score_label:
					score_label.text = "BEST: %06d" % high_sc
			else:
...
func update_shop_ui() -> void:
	...
	if GameManager.upgrade_hp < hp_cost.size():
		...
		if buy_hp_btn:
			buy_hp_btn.text = "UPGRADE (+25 HP)\n💰 %d" % cost
			buy_hp_btn.disabled = GameManager.coins < cost
	else:
	...
	if GameManager.upgrade_speed < speed_cost.size():
		...
		if buy_speed_btn:
			buy_speed_btn.text = "UPGRADE (+SPEED)\n💰 %d" % cost
			buy_speed_btn.disabled = GameManager.coins < cost
	else:
	...
	if GameManager.upgrade_bombs < bomb_cost.size():
		...
		if buy_bombs_btn:
			buy_bombs_btn.text = "UPGRADE (+1 BOMB)\n💰 %d" % cost
			buy_bombs_btn.disabled = GameManager.coins < cost
	else:

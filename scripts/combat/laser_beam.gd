...
@export var damage_per_second: float = 450.0
...
func _process(delta: float) -> void:
	...
	# Force strictly vertical rotation (0 degrees straight UP)
	global_rotation = 0.0
	update_beam_geometry()

	# Pulse beam width & alpha for high-tech energy beam effect
	var pulse = 1.0 + sin(Time.get_ticks_msec() * 0.035) * 0.18
	if beam_core: beam_core.width = 14.0 * pulse
	if beam_glow: beam_glow.width = 36.0 * pulse
...
func create_hit_spark(pos: Vector2) -> void:
	...
	spark.modulate = Color(0.3, 2.8, 3.5, 0.95)
	get_parent().add_child(spark)
	
	var tween = spark.create_tween()
	tween.tween_property(spark, "scale", Vector2(0.8, 0.8), 0.08)
